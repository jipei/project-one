...
public class CartServiceImpl implements CartService {
    ...
    public List<Cart> addItemToCartList(List<Cart> cartList, Long itemId, Integer num) {
        ...
        if (cart == null) {
            ...
            TbOrderItem orderItem = findOrderItemByItemByItemId(cart.getOrderItemList(), itemId);
            if (orderItem !=null){
                //商家（cart）存在；商品（tbOrderItem）存在的话；那么购买数量叠加并跟新总价；
                ...
                //商家（cart） 存在；商品（toOrderItem）不存在的话；重新创建商品加入商品列表
                if (num>0){
     ...
     * @return 订单商品
     */
    private TbOrderItem findOrderItemByItemByItemId(List<TbOrderItem> orderItemList, Long itemId) {
